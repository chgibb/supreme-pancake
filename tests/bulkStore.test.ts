...
it(`should write bulk test data to its own directory`,async () => {
    ...
    expect(await bulkStoreTweets("tests/rt/bulkStoreTS2",tweets,"out/")).toEqual(<BulkStoreStatus>{added:5039,duplicates:501,success:1});
});

it(`should merge bulk test data`,async () => {
    let tweets = fs.readFileSync("tests/res/bulkStore.json").toString();
    expect(await bulkStoreTweets("tests/rt/bulkStoreTS",tweets,"out/")).toEqual(<BulkStoreStatus>{added:5039,duplicates:501,success:1});
});

it(`should not add duplicates`,async () => {
    let tweets = fs.readFileSync("tests/res/bulkStore.json").toString();
    expect(await bulkStoreTweets("tests/rt/bulkStoreTS",tweets,"out/")).toEqual(<BulkStoreStatus>{added:0,duplicates:5540,success:1});
    expect(await bulkStoreTweets("tests/rt/bulkStoreTS",tweets,"out/")).toEqual(<BulkStoreStatus>{added:0,duplicates:5540,success:1});
    expect(await bulkStoreTweets("tests/rt/bulkStoreTS",tweets,"out/")).toEqual(<BulkStoreStatus>{added:0,duplicates:5540,success:1});
    expect(await bulkStoreTweets("tests/rt/bulkStoreTS",tweets,"out/")).toEqual(<BulkStoreStatus>{added:0,duplicates:5540,success:1});
    expect(await bulkStoreTweets("tests/rt/bulkStoreTS",tweets,"out/")).toEqual(<BulkStoreStatus>{added:0,duplicates:5540,success:1});
