...
[[nodiscard]] std::string PanCake::makeTweetTimePointPath(const char*path,const PanCake::Tweet&tweet)
{
    return std::string(path) + "/" + tweet.date;
}

[[nodiscard]] std::string PanCake::makeTweetTimePointBinPath(const char*path,const PanCake::Tweet&tweet)
{
    return PanCake::makeTweetTimePointPath(path,tweet) + "/" + tweet.userHash[0] + ".json";
}

[[nodiscard]] std::string PanCake::makeTweetImagePath(const char*path,const PanCake::Tweet&tweet)
{
    return PanCake::makeTweetTimePointPath(path,tweet)+"/img";
}

[[nodiscard]] std::string PanCake::makeTweetImageFilePath(const char*path,const PanCake::Tweet&tweet,int index)
{
...
[[nodiscard]] std::vector<std::string> PanCake::makeTweetImageFilePathsForImageContent(const char*path,const PanCake::Tweet&tweet)
{
    std::vector<std::string> res;

    int i = 0;
    std::for_each(
        tweet.imageContent.begin(),
        tweet.imageContent.end(),
        [&tweet,&path,&res,&i](const PanCake::TweetImageContent&img) -> void {
            res.push_back(PanCake::makeTweetImageFilePath(path,tweet,i));
            i++;
        }
    );
