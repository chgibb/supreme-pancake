...
namespace PanCake
{
    class ChunkableFavouriteCount : public PanCake::ChunkableColumn
    {
        public:
            ...
            {
                this->addValueToChunk(this->contName,tweet.favouriteCount);
            }
            ...
            {
                return std::ofstream(PanCake::ChunkableFavouriteCount::makeOutPutPath(prefix,date),std::ios::out);
            }

            static std::string makeOutPutPath(std::string prefix,PanCake::TweetDate&date)
            {
                return PanCake::makeColumnIROutputPath(prefix,date,"favouriteCount");
            }
    };
}
